...
<html>
    ...
    <body>               
        ...
        $key = array_search($data[0] . '.php', $file);
        if ($data[0] == '') {
            echo '<p>упс что-то пошло не так</p>';
            echo '<p>Название услуги является обязательным полем =( </p>';
            echo '<div><input class="button" type="button" onclick="history.back();" value="Вернуться на шаг назад"/></div>';
        } elseif ($key > '0') {
            echo '<p>упс что-то пошло не так</p>';
            echo '<p>Название услуги <i>' . "$data[0]" . '</i> уже используеться =( </p>';
            echo '<div><input class="button" type="button" onclick="history.back();" value="Вернуться на шаг назад"/></div>';
        } else {
            ...
            echo '<div><a class="button" href="position.php">Добавить еще позицию</a></div>';
        }
        ?>
